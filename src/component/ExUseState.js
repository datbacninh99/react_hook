...
export default function ExUseState() {
    ...
    // Sử dụng useEffect
    // 1. Sử dụng useEffect(callback) --> được gọi khi render(componentWillMount) hoặc re-render(componentWillUpdate)
    useEffect(() => {
        console.log('useEffect(callback) được gọi khi render và re-render');
    })
    // 2. Sử dụng useEffect(callback, []) --> được gọi khi render(componentWillMount) hoặc re-render(componentWillUpdate)
    useEffect(() => {
